                  ...
                  <CardContent className="p-4">
                    <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 ">
                      ...
                      <div className="flex flex-col sm:flex-row gap-2 self-end lg:self-center">
                        <Badge variant="outline" className="bg-yellow-100 border-yellow-300 text-yellow-700 w-fit">
                          Pending
                        </Badge>
      ...
      {selectedPayout && !showApproveDialog && (
        <Dialog open={!!selectedPayout} onOpenChange={closeDialogs}>
          <DialogContent className="max-w-2xl">
            ...
            <div className="space-y-6 py-4">
              <div className="space-y-4">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-slate-800 font-medium">Doctor Information</h3>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <p className="text-sm font-medium text-muted-foreground">Name</p>
                    <p className="text-slate-800">Dr. {selectedPayout.doctor.name}</p>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-muted-foreground">Email</p>
                    <p className="text-slate-800">{selectedPayout.doctor.email}</p>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-muted-foreground">Specialty</p>
                    <p className="text-slate-800">{selectedPayout.doctor.specialty}</p>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-muted-foreground">Current Credits</p>
                    <p className="text-slate-800">{selectedPayout.doctor.credits}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-slate-800 font-medium">Payout Details</h3>
                </div>
                <div className="bg-emerald-50 p-4 rounded-lg border border-emerald-200 space-y-3">
                  <div className="flex justify-between">
                    ...
                    <span className="text-slate-800 font-medium">{selectedPayout.credits}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Gross amount (10 USD/credit):</span>
                    <span className="text-slate-800">${selectedPayout.amount.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Platform fee (2 USD/credit):</span>
                    <span className="text-slate-800">-${selectedPayout.platformFee.toFixed(2)}</span>
                  </div>
                  <div className="border-t border-emerald-200 pt-3 flex justify-between font-medium">
                    <span className="text-slate-800">Net payout:</span>
                    <span className="text-purple-500">${selectedPayout.netAmount.toFixed(2)}</span>
                  </div>
                  <div className="border-t border-emerald-200 pt-3">
                    <p className="text-sm font-medium text-muted-foreground">PayPal Email</p>
                    <p className="text-slate-800">{selectedPayout.paypalEmail}</p>
                  </div>
                </div>
              </div>
              ...
              {selectedPayout.doctor.credits < selectedPayout.credits && (
                <Alert variant="destructive">
                  ...
                  <AlertDescription>
                    Warning: Doctor currently has only {selectedPayout.doctor.credits} credits
                    but this payout requires {selectedPayout.credits} credits. The payout cannot
                    be processed.
                  </AlertDescription>
                </Alert>
              )}
            ...
            <DialogFooter>
              <Button variant="outline" onClick={closeDialogs} className="border-emerald-900/30">
                Close
              </Button>
              <Button
        ...
        <Dialog open={showApproveDialog} onOpenChange={() => setShowApproveDialog(false)}>
          <DialogContent>
            ...
            <div className="space-y-4 py-4">
              <Alert>
                ...
                <AlertDescription>
                  ...
                  <ul className="mt-2 space-y-1 list-disc pl-4">
                    <li>Deduct {selectedPayout.credits} credits from Dr. {selectedPayout.doctor.name}'s account</li>
              ...
              <div className="bg-emerald-50 p-4 rounded-lg border border-emerald-200">
                <div className="flex justify-between mb-2">
                  ...
                  <span className="text-slate-800">Dr. {selectedPayout.doctor.name}</span>
                </div>
                <div className="flex justify-between mb-2">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-slate-800 text-sm">{selectedPayout.paypalEmail}</span>
                </div>
              </div>
            </div>
            ...
            <DialogFooter>
              ...
              <Button onClick={confirmApproval} disabled={loading} className="bg-purple-600 hover:bg-purple-700">
                {loading ? (
                  <>
